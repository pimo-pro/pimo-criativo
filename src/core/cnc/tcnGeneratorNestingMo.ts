/**
 * Sistema TCN NESTING MO.
 *
 * Neste momento é uma cópia literal do `v3_new`, em ficheiro separado,
 * para evoluir como modo principal do projeto.
 */
...
function resolveContourToolOffsetMm(settings: {
  cnc?: { diametroFresaContornoMm?: number; compensacaoFerramenta?: "fora" | "dentro" };
}): number {
...
export function generateTcnForPanelNestingMo(
  ...
  const TOOL_DIAMETER_MM = getContourToolDiameterMm(runtimeSettings);
  const TOOL_RADIUS_MM = TOOL_DIAMETER_MM / 2;
  const contourToolOffsetMm = resolveContourToolOffsetMm(runtimeSettings);

  const MIN_SPACING_MM = Number.isFinite(Number(runtimeSettings?.cnc?.minSpacingMm))
    ? Math.max(0, Number(runtimeSettings?.cnc?.minSpacingMm))
    : DEFAULT_MIN_SPACING_BETWEEN_PIECES_MM;
  const SHEET_MARGIN_MM = Number.isFinite(Number(runtimeSettings?.cnc?.sheetMarginMm))
  ...
  const pushContourFromPath = (
    ...
    const w89T = transformPlacementToTcn({ x: w89x, y: w89y, z: Z_SAFETY_MM }, dl, maxW, maxH);
    const pathT = result.path.map((p) => transformPlacementToTcn(p, dl, maxW, maxH));
    sideInnerLines.push(buildToolBlock(w89T.x, w89T.y, Z_SAFETY_MM));
    sideInnerLines.push(buildW2201(pathT, Z_SAFETY_MM));
  };
  ...
  for (const pl of sanitizedPlacements) {
    ...
    pushContourFromPath(
      buildPlacementExteriorContourPath(pl, contourToolOffsetMm, panelMm, Z_SAFETY_MM, RAMP_DISTANCE_MM)
    );
    for (const inner of buildPlacementInnerContourPaths(pl, TOOL_RADIUS_MM, zCut, Z_SAFETY_MM)) {
