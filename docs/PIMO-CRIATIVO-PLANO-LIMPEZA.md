...
| Campo | Valor |
|-------|--------|
| **Versão do plano** | 1.13 |
| **Estado** | Fases **1.3–1.12 (L-03 → L-30) executadas** em 18 de Agosto de 2026 |
| **Modo actual** | Pós-execução das fases L- aprovadas; restantes IDs continuam em planeamento |
...
| **Próximo passo** | Fases L-01/L-02, 1.13 (L-12/L-13) e 1.14 (L-18/L-20) **não** executadas — aguardam gatilho explícito |
...
| **Z-** | **Zona dormida** — código interno sem UI/runtime externo; não apagar sem auditoria de cadeia | `industrial/core/dashboard` |
...
| L-23 integration/ui | **Z-01** | Cadeia interna; zero imports em `app/` |
...
| F-08 | `Viewer.events.emit` | Stub (auditoria viewer) | No-op | Ruído API |
...
## 6. Zonas dormidas (`Z-`) — não mortas; auditoria antes de remover

| ID | Item | Caminho | Evidência | Porque não é L- |
|----|------|---------|-----------|-----------------|
| Z-01 | Integration UI industrial | `src/industrial/integration/ui/*` | Zero imports em `src/app` | Tipos re-exportados; possível uso futuro TRAK |
...
| R-05 | ViewerCore monolítico | ~6112 linhas | **Alta** manutenção | Fase 5 only |
...
src/industrial/integration/ui/**          [Z-01]
...
R-05, D-09, D-10, smoke ViewerCore, R-08, R-10, E2E mínimo.
...
2. Pedir execução **explícita** (ex.: «aplica Fase 1.11 — L-29» ou «aplica Fase 1.12 — L-30»). A regra L- **não** substitui este gatilho.
...
Fim do documento de planeamento (v1.13).
